...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.headerReferenceSize = CGSizeMake([[UIScreen mainScreen] bounds].size.width, 65.0f);//头部视图的框架大小
        
        self.itemSize = CGSizeMake([[UIScreen mainScreen] bounds].size.width/7, 70.0f);//每个cell的大小
